...
export const EVALUATION_PROMPT_VERSION = "v3.6";
...
- \`book\`: the book, named. Short. "Philippians." Not an argument and not a score. Stored; not a display row.
- \`passage\`: one sentence. The theme / big idea of *this passage*.
- \`melodic_line\`: one sentence. **Always begin by naming the book in the first clause**, then the unifying theme as this report reads it. Right: "Hebrews keeps returning to the superiority of Christ over every prior covenant mediator..." Wrong: "Christ is better than every good thing that came before him, so do not drift back..." The book name is required because nothing else on the page will name it. If the preacher named a working line, that line is the premise; restate it, still naming the book first. **Exception — withheld or topical:** if you will not invent a line, or the preacher has indicated a topical message with no single book's line, write that explanation as you do now and do not force a book name into the opening clause.
