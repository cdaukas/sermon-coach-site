...
4. When a hinge is available, let it carry the score grammar (load-bearing contrast or concession):
   ...
   - Mid scores (3 to 4): concession. The strength holds, but a named cost remains when the narrative has that cost.
   - Low scores (1 to 2): lead with the problem; any credit second when the narrative has both.
...
5. Elision between clauses is allowed; elision within one clause is not. Write "named, but not pressed". Never write "named not pressed."
...
8. House rules: the em-dash (—) and the en-dash (–) are forbidden. So is an unspaced double-hyphen. Recast with a comma, a period, or a semicolon. Do not swap in a hyphen. No exclamation points.
...
export function buildVerdictLineQualityRetryNote(
  ...
  const bullets = issues.map((issue) => {
    ...
    if (issue.reason === "em_dash") {
      return `Criterion ${issue.id} (score ${issue.score}/5): ${issue.detail}. Recast with a comma, a period, or a semicolon. Do not use an en-dash or a hyphen. Failed line: "${preview}"`;
    }
