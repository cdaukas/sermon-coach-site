...
- **No direct quotes from the sermon in the verdict.** Quotes are body work — they belong in the per-criterion narratives, the "Lead with these" cards, the rewrites. The verdict names the move at a higher altitude: "the exegetical refusal to caricature Moses" rather than `"this is not bad versus good, this is good versus better"`. Quoting in the verdict makes it feel like the report opening; the verdict is supposed to feel like the report's pastoral frame.
- **Affirmation names ONE strength, not three or four.** Pick the single strongest move in the sermon. If the report has four "Lead with these" cards, the verdict names one of them — the one that most defines this sermon's character. Stacking multiple affirmations turns the verdict into a list and dilutes the praise. One named strength carries more weight than four.
- **Two paragraphs, structured this way:**
  ...
  - *Paragraph break, then second paragraph (~15-20 words, one improvement — a headline pointer, not an explanation):* Open with a phrase like **"The one thing that would take this from solid to exceptional:"** or **"The single highest-leverage change for the next sermon:"** — then name ONE thing in a single short sentence. No expansion, no examples, no rationale. This paragraph functions as a typographic flag, not a mini-argument. Top 3 Priorities #1 will do the explaining — the verdict's job is just to point. If your improvement paragraph is running past 20 words, you're doing Priority #1's job in the wrong location.

  **Typography for the improvement paragraph (locked, both markdown and HTML):**
  - The opener phrase ("The single highest-leverage change for the next sermon:" or equivalent) is **bolded** — it functions as a typographic flag so the reader's eye finds the one improvement instantly.
  - The body of the paragraph (everything after the colon) is **regular weight, NOT italicized**. The full paragraph italic treatment is wrong — it makes the improvement read as a hedged aside rather than the load-bearing diagnostic claim it is. The paragraph carries the same visual weight as the affirmation paragraph above it; only the opener is set apart.
  - In HTML, this means: do NOT apply `font-style: italic` to the improvement paragraph's container. Bold the opener inline (`<strong>` or equivalent). The paragraph should otherwise inherit the verdict's default body styling.
  - In markdown, this means: `**The single highest-leverage change for the next sermon:**` followed by regular prose. No surrounding underscores or asterisks on the body.
...
4. **"Lead with these"** section (What's Working — affirmation half of the body)
...
- `--green: #4a7c59` — sliders 4-5, "Lead with these" card top borders
- `--green-soft: #7ba886`
...
- **The Heat Map is audio-conditional — never fake one from a manuscript.** If audio/video isn't available, omit the Heat Map section entirely. Criterion #8 stays in the rubric as a scored slider (capped at 4/5 for manuscript-only), and its narrative carries the diagnostic work in prose. A manuscript-inferred heat map adds visual weight without earning it.
...
7. The Overall Verdict is ~70-85 words total across two paragraphs (affirmation ~50-60 words, improvement ~15-20 words as a single short sentence — a headline pointer, not an explanation). It contains NO direct quotes from the sermon — quotes are body work. The affirmation names ONE strength (the single strongest move in the sermon), not three or four. If the verdict is longer than 95 words, names multiple strengths, runs the improvement paragraph past 20 words, or contains quotation marks pulled from the sermon, it's doing the body's job — rewrite it shorter and higher-altitude before finalizing.
7a. **Verdict improvement paragraph typography:** opener phrase is bolded (`**The single highest-leverage change for the next sermon:**` or equivalent), the body of the paragraph after the colon is regular weight and NOT italicized. In HTML, no `font-style: italic` on the paragraph container. The improvement paragraph carries equal visual weight to the affirmation; only the opener is typographically flagged.
...
9. The report follows the section order in BOTH markdown and HTML: (1) composite score + verdict at top; (2) four category dashboards; (3) heat map (audio only — omit for manuscript); (4) "Lead with these"; (5) "Where You Can Grow" (sole prescriptive section); (6) "What Improvement Looks Like"; (7) Methodology appendix at the end. Category subtotals and the simple-vs-weighted breakdown live in the Methodology appendix, NOT at the top.
10. The three section titles are locked: **"Lead with these"** for What's Working, **"Where You Can Grow"** for Top 3 Priorities, **"What Improvement Looks Like"** for Suggested Rewrites. No editorial garnish, no alternative phrasings.
...
**Manuscript-only evaluations cap below 55.** Criterion 8 (Heat Map: emotional delivery) cannot honestly score a 5 from a manuscript alone — it is a *delivery* criterion, and a document can show only that the emotional arc is well **designed**, not that it is well **delivered**. A manuscript-only evaluation therefore tops out around 53/55, and a true 5 on criterion 8 — and so any score in the high A range — requires audio or video of the sermon as actually preached. When a preacher working from a manuscript is chasing a top score, say this plainly: the unscoreable delivery criterion is a limit of the medium, not a flaw in the sermon, and the realistic ceiling for a document evaluation is the low A band. Do not inflate criterion 8 to a 5 to close the gap.
...
Do the emotional beats of the sermon — lament, joy, warning, comfort, conviction, awe — get delivered with matching tone, volume, pacing, and body language? This is a *delivery* criterion, scored from audio or video when available. For manuscript-only evaluations, the criterion is capped at 4/5 — a document can show whether the emotional arc is well *designed*, not whether it is well *delivered*. Mismatch between content and delivery (preaching about glory in a flat monotone; preaching about lament with a smile) is a delivery failure even when the content is right.
...
- **Heat Map is now audio-conditional.** The Heat Map subsection (markdown) and Heat Map panel (HTML) are included only when audio or video of the preached sermon is available. Manuscript-only evaluations omit the section entirely — criterion #8 (Heat Map: emotional delivery) stays in the rubric as a scored slider (capped at 4/5), and its expanded narrative does the diagnostic work in prose. The heat map was always meant to score *delivery*; on a manuscript it became textual-cue speculation dressed up as a timeline. This change preserves the rubric (11 criteria, 55 points) while letting the visual real estate earn its space only when it can do real work.
...
- **Verdict word budget tightened from 80-100 to 70-85 words**, with the improvement paragraph dropping from 25-35 words to 15-20 words as a single short sentence (headline pointer, not explanation). Top 3 Priorities #1 does the explaining; the verdict's job is just to point. Hard cap moves from 110 words to 95.
...
- **Interpretive guidance added (Grading bands section):** the A-Exemplary band (47–55), not a perfect 55, is the ceiling worth aiming at — chasing all-5s overstuffs a sermon. And manuscript-only evaluations cap around 53/55, because criterion 8 (emotional delivery) needs audio or video to score a 5.
...
- Audio or video of the delivered sermon (required for the Heat Map section to be included; without it, the section is omitted and criterion #8 is capped at 4/5)
