...
7. The Overall Verdict is ~70-85 words total across two paragraphs (affirmation ~50-60 words, improvement ~25-30 words with a ≤32 hard cap, as a single short sentence — a headline pointer, not an explanation). It contains NO direct quotes from the sermon — quotes are body work. The affirmation names ONE strength (the single strongest move in the sermon), not three or four. If the verdict is longer than 95 words, names multiple strengths, runs the improvement paragraph past 32 words, or contains quotation marks pulled from the sermon, it's doing the body's job — rewrite it shorter and higher-altitude before finalizing.
