...
### D2. Does the prompt penalize humor? Unverified, free to check.

The corpus says self-aimed humor costs the preacher and is evidence of pastoral warmth: phi ≈ 0.73 across two corpora and 1,757 coded instances, the best-powered finding in this document. The trivializing half is **undetectable** — both candidate predictors reversed sign between reads.

**If the current prompt or any band definition penalizes levity, it is producing wrong coaching today** on a criterion where the corpus says Chris is strong. Read the prompt, log severity here, do not fix yet.

### D3. Does gospel clarity score on features that do not track load-bearingness? Unverified, free to check.

Load-bearing versus removable has **no surface signature**: four instruments returned r between −0.09 and +0.04, and position is uninformative at r = +0.038. If criterion 3's scoring rests on any textual or positional feature, it is measuring something other than what it claims.

---

## B. Improvements. These wait for the bump.

### From application (criterion 7)

**Add the three-part tell to the narrative.** Object supplied, cost named, objection anticipated. Tells a preacher *why* he received a 3 rather than only that he did. Closest to ready of anything here. Should inform the narrative without changing the number.
...
**The conclusion asymmetry is a near-law, not a tendency.** 51 to 6, McNemar p < 0.001. If criterion 5's scoring does not weight the ending, it is missing the most reliable structural fact in the corpus.
...
**TRD and L6 are shippable as observables**, with rule 12: a hit means look at the ending, not the ending is broken. They cannot tell a collapse from a crescendo.
