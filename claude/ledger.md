...
**This ledger and the priorities board now live in the repo, committed at `a973306` on 5 Sept.** That closes the item that existed because the ledger had forked twice. **The fix only holds if edits go through the repo copy.** It forked again the same day, once through a corpus session and once through a chat thread patching an uploaded snapshot, and the two did not know about each other. Confirm the committed copy is current rather than an earlier paste before editing, and treat any uploaded snapshot as stale by default.
...
| Packs | Stay personal property. Never team-owned, never pooled. Founding Member 50% never applies. **Packs are for preachers who preach less than once a month, decided 5 Sept.** Two and six fit; twelve never did. The defense of packs is validity, not price: a guest preacher pays $58 across two 2-packs against $348 to subscribe. Per credit they run $14.50 and $11.50 against Coach at $2.90 at full use. |
| **Teams stays separate from Classroom or Preaching Lab** | **Decided 5 Sept, and this is the argument that should stop anyone merging them.** Teams is an ongoing subscription billed monthly or annually like Coach. Classroom or Preaching Lab is billed by the term: the term ends, the roster empties, the next class is a new purchase. That is two billing shapes, not two taglines over one product, which is what Classroom and the Lab turned out to be. It also explains the free-seat asymmetry: the Teams lead pastor pays $29 because he is a Coach subscriber with a team attached; the Lab organizer's seat is free because he is an instructor for a term. **Two Tally forms is correct here** for the same reason. |
| **Teams annual** | Two months free across the whole invoice, same rule as Coach, **no cap and no seat-count threshold**. Consistency beats margin protection at current volume. The upsell happens in the quote email, not the Tally form: a buyer filling out intake has not been quoted yet and cannot choose blind. The real reason to want annual is not the cash, it is that a church on an annual term does not reconsider in March. |
| **The lead pastor's $29 is always charged, never comped at a seat threshold** | Considered 5 Sept and rejected. Comping the Coach line above five seats **inverts the price**: four Colleague seats plus $29 is $129, five with the seat comped is $125, so adding a man lowers the bill. At Apprentice it is worse, $77 against $60. Any threshold that gives away something worth more than the marginal unit creates a cliff. A comp granted by hand in conversation is a favor; a comp on the card is a threshold buyers optimize against. |
...
**The pricing page rebuilt itself in one afternoon.** 5 Sept, six PRs. Ordered: #329 footer sketch link, #330 homepage *Develop others*, #331 packs collapsed at `61cb29f`, #332 Coach block at `a8c6a3b`, #333 credits two links at `a122444`, #334 Classroom and Lab merged at `0c88f48`. Eight public tiers became six with nothing removed that anyone was buying.

**Coach stopped looking stranded, and annual became the default.** #332 removed the *Most Preachers* ribbon, which had been comparing Coach to a sibling deleted in #331, and narrowed `.subs-solo` from 900px to 620px. The cadence control now initialises to annual. **The annual price is framed monthly: $24.17 /mo, with "2 months free. Billed annually at $290." on one line below it.** $290 ÷ 12 = $24.1666; $29 × 12 = $348, less $290, is $58, exactly two months. **The disclosure line is the whole defense of this framing.** A monthly figure against a yearly charge is the standard soft trick in SaaS pricing, and the only thing separating this from it is that line sitting at the same size on the same line. It must never shrink, gray out, or move below the fold. It currently renders in `--sc-accent`, so when the AA branch ships it should move to `--sc-ink-soft`, not to a darker gold.

**I shipped a bug in #331 and #333 fixed it.** Collapsing the pack toggle removed the 2-pack's only entry point, so the block offered two prices with one reachable. Prices moved out of the prose and onto two links, `2 credits · $29` and `6 credits · $69`. **Live `pack_2` was confirmed at $29 and active in the Stripe dashboard before shipping**, after a first attempt tried to proceed on a test-mode `unit_amount` and a marketing claim. Fixing an unreachable link with a second unreachable link would have been the same error twice.

**Classroom and Preaching Lab merged into one card, and that closes item 15 completely.** #334. Both CTAs already pointed at the same Tally form, so the 30 Aug one-form decision was already true in the plumbing and two cards was the leftover. **The merge superseded item 15's remaining bullet rather than shipping it:** the briefed copy said smaller groups run as a Preaching Lab, and there is no separate Lab card to route them to. The five-seat figure is gone from the page entirely. The Lab also had no orientation router entry, so it had been live with no path to it from the top of the page; the merged entry fixes that.

**The `PRICING.cohort` sync is the reason #334 worked.** Cursor's Step 0 caught that `applyBillingCadence` writes `data-price|period|note|tier-cta="cohort"` to the Classroom card, so markup alone would have looked right in the file and had "Five-seat minimum" and "/student/mo" written back over it on load. Verified on production by raw-HTML grep: `Five-seat minimum` 0, `student/mo` 0, `preacher/mo` 3, which is the markup plus both cadence entries.

**Series Prep is genuinely gone, not half-retired.** Item 26. The Stripe price is archived and `pack_12` is out of `checkout.ts` in the same pass, because archiving alone would have routed anyone with an old Payment Link to a dead price. Migration CHECK constraints were left alone; they cost nothing and record history honestly. **Note for any future retirement:** unauthenticated, `/checkout?pack=<anything>` redirects to `/signup?pack=<same>` before validation runs, so a retired param cannot be verified from outside and a stale link walks a person through account creation before it fails. Verify logged in.

**Packs collapsed, Series Prep retired.** 5 Sept, **PR #331**, merged at `61cb29f`, branched from `a973306`.

**The state before was not three pack cards.** The pricing page already carried one Pack card with a three-way toggle, 2 / 6 / 12, defaulting to Pulpit Supply. The real error was upstream: the orientation router listed *Pack* as a peer of *Coach* under "For your own preaching," giving a $69 one-time purchase equal billing with a $348-a-year subscription. Deleting that entry was the highest-value line in the item and it was a deletion.

**Why Series Prep had to go, and the rule it leaves behind.** Per credit, packs run $14.50, $11.50 and $9.08 against Coach at $2.90 at full use and $9.67 even for a pastor using three a month. Price was never the defense of packs; **validity is.** A guest preacher preaching four times a year pays $58 across two 2-packs against $348 to subscribe, and packs win decisively for that man. Twelve credits across 18 months is eight a year, which is monthly preaching: four months of Coach costs $116 against Series Prep's $109 and hands him 40 credits rather than 12. **Series Prep intercepted the best subscription candidate on the page and sold him a worse deal than the thing he should have bought.** The rule that survives: **packs are for preachers who preach less than once a month.** Two and six fit that definition; twelve never did.

**What replaced the card**, directly beneath Coach, no card frame, no toggle, no badge, no per-credit math: *"Preaching a few times a year? Buy credits instead of subscribing. Two for $29, six for $69. Same rubric, same evaluation, valid 18 months, and they keep working if you subscribe later."* Per-credit arithmetic was deliberately dropped with the card, because it invites a comparison against Coach that Coach wins so decisively it argues against the block it sits in.

**Two Step 0 findings corrected my brief, and one of them mattered.** `applyPackSize` is never called on load; the 1269/1271 rewrite belongs to `applyBillingCadence` and the Coach cadence toggle, so static markup holds for the pack CTA. And **`.pack-size-*` CSS is shared with the mentoring seat toggle while `data-pack-size` is pack-only.** Deleting on the class rather than the hook would have taken a $12 sale down with a $69 one. All CSS was left untouched and a scoped `.credits-note` rule was added instead.

**Dead JS was removed rather than left unreachable**, including the `PACKS` object and the Series Prep Payment Link in an HTML comment. Both would have survived a `perl` check that strips `<script>` blocks, which is why the verification ran twice, once on visible copy and once on raw HTML.

**Verified independently on production.** Raw HTML: `Series Prep` 0, `pack_12` 0, `data-pack-size` 0. Beyond the brief: `applyBillingCadence` 3 and `data-mentor-seat` 4, so neither toggle was collateral damage, and *Occasional and milestone* 0, so the router entry is gone.

**Deliberately left alive:** `checkout.ts`, `pack-credits.ts`, `checkout.test.ts` and the migration CHECK constraints. Retiring a product from the storefront is not revoking what people bought, and the 18-month validity is a promise.

**Open, and my archive advice was incomplete.** `/checkout?pack=pack_12` still resolves to a live $109 price. Archiving the Stripe price alone routes anyone with an old Payment Link to a dead price. Nobody holds unused Series Prep credits, so the call is genuinely gone: archive the price **and** pull `pack_12` from `checkout.ts` in the same pass. Leave the migration constraints; they cost nothing and record history honestly.
...
- **A shared class name is not a shared JS hook, and the reverse.** `.pack-size-*` CSS is shared with the mentoring seat toggle while `data-pack-size` is pack-only. Check both before deleting either, or a cleanup on one product takes down another product's control.
- **Verify a retirement against raw HTML, not only visible copy.** Dead script blocks and HTML comments survive a `perl` pipe that strips `<script>`, so a retired SKU can read as gone while its Payment Link still sits in the file.
- **Archiving a Stripe price is half a retirement.** If a route still resolves to the archived price, the buyer meets a dead checkout instead of a redirect. Archive and remove the route in one pass.
- **Two cards with different price grammar invite a false comparison.** Teams headlining a bundled $54 beside a per-unit $12 reads as four and a half times the cost. Both must headline the same unit before they sit side by side.
- **A layout that hides an overlap is not a fix.** Stacking Teams above the merged card concealed that the same four-preacher church prices cheaper next door. Making them peers exposed it, which is how the term-versus-subscription distinction got found.
