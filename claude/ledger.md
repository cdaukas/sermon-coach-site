...
Last updated 5 September 2026, late.
...
| **Per-criterion inventory** | `claude/inventory-per-criterion-reports.md` | 6 Sept. **What each criterion has to work with.** Measures, quotable spans, rewrite targets, interpretation material. The input to any further report scoping. |
| **Movement report spec** | `claude/spec-movement-report.md` | 6 Sept. The second half of the six-month arc. |
| Interpretation copy | `claude/prep-card-interpretation-copy.md` | Nineteen paragraphs, high and low, written against the voice profile. |
...
### The delivery model. Four themes, eight quarters, two years. Decided 5 September.

**Rotation is by theme, not by criterion.** The eleven criteria are a scoring instrument, not a curriculum. Five of them have one measure or none, so a criterion rotation thins out fast. Four themes, each built from measures that actually exist, do not.

| theme | measures | criteria touched |
|---|---|---|
| **The ask** | named object, named cost, reciprocal, naming valence | 9, 10 |
| **Christ in the sermon** | agency, Christ in a main point, the cross's named object, gospel in the outline | 2, 3 |
| **Who is in the room** | whose condition, non-Christian address, the two-marker screen | 4, 7, 9 |
| **Delight and difficulty** | the grain tell, object migration, softening moves, unresolved hard things | 6, 11 |

**Chris's structure, and it is better than the quarterly rotation I proposed.** Each theme runs **six months, not three**: a diagnostic report names three things, the preacher works a quarter with the prep card carrying the three, then a **movement report** compares the original sermon set against only the sermons preached since.

Twelve weeks is not long enough to change how a man writes on Saturday. Twenty-four is.

**Four themes × six months = two years.** Eight reports, every one with a job.

**The prep card's role is settled by this.** It carries the three disciplines through the work quarter. The report diagnoses and answers; the card sits on the desk in between. They stop competing.

**Excluded from the rotation, deliberately.** Criterion 5, structure, has no rewrite target and is a prep-lens instrument measuring whether the preacher stopped writing. It belongs on the card. Criterion 8, emotional arc, has no reliable per-sermon evidence and is already roughly calibrated. **Padding either in would be inventing measures to fill slots**, which is the failure the corpus method exists to prevent.

**Criterion 7 has no material of its own** and needs the corpus read the other ten had, framed on Keller's three audiences.

### The application report is built, and it is the quarterly report

**5 September.** Read, revised through six rounds, and Chris's verdict on the finished version: it is the quarterly report, and the prep card becomes a condensed desk version of it.

What shipped: five strength quotes across four sermons, three focus items with numbered growth questions, an interpretation paragraph per measure, a Was/Now rewrite constrained to fix its own marker, and a three-quote pattern list under each focus. Roughly $0.01–0.03 per card for one Sonnet rewrite call.

**Gated to Chris only** via `profiles.prep_card_access`, mirroring the Growth gate. Page, rail and build action all check the flag.

**Six revisions, and what each one found.** Quotes rendering from the wrong ask. Mode counts printed as facts while moving a fifth of the sample between runs. A model writing a finding nothing had measured. The object marker at 38 of 57 when witnessability says 6. A staging note and a file header selected as asks. Rewrites polishing the sentence instead of fixing the failed marker.

**The strengths floor works.** Only one measure cleared 50% of its eligible sample, and the card says so rather than promoting a 4-of-18 into praise.

### Series Prep in Stripe### Series Prep in Stripe. One decision, outside the repo.
...
- **A rewrite must fix the failed marker, not improve the sentence.** The prep card's first rewrites produced warmer versions of the same ask: the cost discipline got no cost, the reciprocal discipline got no second person. Passing the specific marker into the prompt with a hard constraint and one worked example fixed it in a single pass.
- **A selector will hand the rewriter anything if you let it.** A staging note about pesos and a file header both got rendered as application asks. Only select spans the coding call marked as asks, and omit the block when none exists.
- **A regex catches the construction and misses the function.** Searching 20 manuscripts for the correction move returned 10 candidates and 3 were real. "That might seem surprising considering how much fruit his life has borne" is the move; "grace of the most surprising kind" is not. Same trigger word, opposite job. **The correction move needs a model, not a parser.**
